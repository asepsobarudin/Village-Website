<div class="w-full">
    <label for="kategori_mobile{{ $title }}"
        class="p-3 my-2 w-full hover:bg-color2 hover:text-color1 rounded-md cursor-pointer flex justify-center items-center">
    ...
    <ul class="hidden flex-col rounded-md p-2 text-color2 justify-center items-center w-full" id="value_kategori_mobile{{ $title }}">
        @foreach ($menu as $mn)
            <li class="flex justify-center items-center my-2 w-full">
                <a href="{{ $mn['link'] }}"
                    class="w-full text-center hover:bg-color2 hover:text-color1 {{ $active == $mn['title'] ? 'text-color3' : '' }} rounded-md p-3">{{ $mn['title'] }}</a>
            </li>
        @endforeach
    </ul>
</div>
